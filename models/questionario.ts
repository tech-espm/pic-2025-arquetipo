...
class Questionario {
  private static validar(questionario: Questionario, criacao: boolean, imagemintroducao?: app.UploadedFile | null, imagemquestionario?: app.UploadedFile | null, imagemlogo?: app.UploadedFile | null): string {
    ...
    if (
      ...
      !(questionario.nomeexterno = questionario.nomeexterno
        .normalize()
        .trim()) ||
    ...
    if (
      ...
      isNaN(parseInt(questionario.iddisponibilidade as any))
    )
    ...
    for (const campo in relacionamentos) {
      if (!questionario[campo] || !(questionario[campo] as any).length) {
        ...
        questionario[campo] = (questionario[campo] as string)
          .split(",")
  ...
  public static async obterPorUrl(url: string): Promise<Questionario | null> {
    return app.sql.connect(async (sql) => {
      const lista: Questionario[] = await sql.query(
        "select id, nomeexterno,iddisponibilidade,anonimo,url,descricao,textointroducao,corfundopagina,corfundocard,cordestaque,cortextocard,cortextodestaque, questoes,versaointroducao, versaoquestionario, versaologo from questionario where url = ?",
        [url]
      );
  ...
  private static async merge(
    sql: app.Sql,
    id: number,
    arquetipos: number[] | null,
    publicosalvos: number[] | null,
    departamentos: number[] | null
  ) {
    if (!departamentos) departamentos = [];

    const existentesDepartamentos: { id: number; iddepartamento: number }[] =
      await sql.query(
        `SELECT id, iddepartamento FROM questionario_departamento WHERE idquestionario = ?`,
        [id]
      );
    const toRemoveDepartamentos = existentesDepartamentos.filter(
      (e) => !(departamentos as number[]).includes(e.iddepartamento)
    );
    const toAddDepartamentos = departamentos.filter(
      (n) => !existentesDepartamentos.some((e) => e.iddepartamento === n)
    );
    const toUpdateDepartamentos: typeof existentesDepartamentos = [];

    while (toRemoveDepartamentos.length && toAddDepartamentos.length) {
      const item =
        toRemoveDepartamentos.pop() as (typeof existentesDepartamentos)[number];
    ...
    for (let i = 0; i < toRemoveDepartamentos.length; i++)
      await sql.query(`DELETE FROM questionario_departamento WHERE id = ?`, [
        toRemoveDepartamentos[i].id,
      ]);

    for (let i = 0; i < toUpdateDepartamentos.length; i++)
      await sql.query(
        `UPDATE questionario_departamento SET iddepartamento = ? WHERE id = ?`,
        [toUpdateDepartamentos[i].iddepartamento, toUpdateDepartamentos[i].id]
      );

    for (let i = 0; i < toAddDepartamentos.length; i++)
      await sql.query(
        `INSERT INTO questionario_departamento (idquestionario, iddepartamento) VALUES (?, ?)`,
        [id, toAddDepartamentos[i]]
      );

    if (!arquetipos) arquetipos = [];

    const existentesArquetipos: { id: number; idarquetipo: number }[] =
      await sql.query(
        `SELECT id, idarquetipo FROM questionario_arquetipo WHERE idquestionario = ?`,
        [id]
      );
    const toRemoveArquetipos = existentesArquetipos.filter(
      (e) => !(arquetipos as number[]).includes(e.idarquetipo)
    );
    const toAddArquetipos = arquetipos.filter(
      (n) => !existentesArquetipos.some((e) => e.idarquetipo === n)
    );
    const toUpdateArquetipos: typeof existentesArquetipos = [];

    while (toRemoveArquetipos.length && toAddArquetipos.length) {
      const item =
        toRemoveArquetipos.pop() as (typeof existentesArquetipos)[number];
      item.idarquetipo = toAddArquetipos.pop() as number;
      toUpdateArquetipos.push(item);
    }

    for (let i = 0; i < toRemoveArquetipos.length; i++)
      await sql.query(`DELETE FROM questionario_arquetipo WHERE id = ?`, [
        toRemoveArquetipos[i].id,
      ]);

    for (let i = 0; i < toUpdateArquetipos.length; i++)
      await sql.query(
        `UPDATE questionario_arquetipo SET idarquetipo = ? WHERE id = ?`,
        [toUpdateArquetipos[i].idarquetipo, toUpdateArquetipos[i].id]
      );

    for (let i = 0; i < toAddArquetipos.length; i++)
      await sql.query(
        `INSERT INTO questionario_arquetipo (idquestionario, idarquetipo) VALUES (?, ?)`,
        [id, toAddArquetipos[i]]
      );
    ...
    const toRemovePublicos = existentesPublicos.filter(
      (e) => !(publicosalvos as number[]).includes(e.idpublicoalvo)
    );
    const toAddPublicos = publicosalvos.filter(
      (n) => !existentesPublicos.some((e) => e.idpublicoalvo === n)
    );
    const toUpdatePublicos: typeof existentesPublicos = [];

    while (toRemovePublicos.length && toAddPublicos.length) {
      const item =
        toRemovePublicos.pop() as (typeof existentesPublicos)[number];
      item.idpublicoalvo = toAddPublicos.pop() as number;
      toUpdatePublicos.push(item);
    }

    for (let i = 0; i < toRemovePublicos.length; i++)
      await sql.query(`DELETE FROM questionario_publicoalvo WHERE id = ?`, [
        toRemovePublicos[i].id,
      ]);

    for (let i = 0; i < toUpdatePublicos.length; i++)
      await sql.query(
        `UPDATE questionario_publicoalvo SET idpublicoalvo = ? WHERE id = ?`,
        [toUpdatePublicos[i].idpublicoalvo, toUpdatePublicos[i].id]
      );

    for (let i = 0; i < toAddPublicos.length; i++)
      await sql.query(
        `INSERT INTO questionario_publicoalvo (idquestionario, idpublicoalvo) VALUES (?, ?)`,
        [id, toAddPublicos[i]]
      );
  }
  ...
  public static async criar(
    ...
    return app.sql.connect(async (sql) => {
      ...
      try {
        ...
        questionario.id = (await sql.scalar(
  ...
  public static async editar(
    ...
    questionario.excluir_imagem_introducao = parseInt(
      ...
      ? 1
      : 0;
    questionario.excluir_imagem_questionario = parseInt(
      questionario.excluir_imagem_questionario as any
    )
      ? 1
      : 0;
    questionario.excluir_imagem_logo = parseInt(
      questionario.excluir_imagem_logo as any
    )
      ? 1
      : 0;
  ...
  private static async atualizarImagem(
    ...
    let versaoAtual: number = await sql.scalar(
      `select ${colunaVersao} from questionario where id = ?`,
      [id]
    );
    ...
    if (excluir) {
      ...
      await sql.query(
        `UPDATE questionario SET ${colunaVersao} = ? WHERE id = ?`,
        [versaoAtual, id]
      );
