...
/*o.addEventListener(je,function(e){e.target===o&&(E.swalCloseEventFinishedCallback(),delete E.swalCloseEventFinishedCallback)})*/

// https://github.com/sweetalert2/sweetalert2/releases/tag/v11.4.19
// Esse trecho está na linha 3098 da versão não minificada -> const animatePopup = (instance, popup, container, returnFocus, didClose) => { ...

// O Swal tem uma animação de 150 ms. Porém, se o elemento for removido da tela
// antes do callback abaixo ser executado, ele não é executado, e o Swal não vai
// completar o processo de limpeza! Então, estou substituindo o callback de término
// da animação por um timeout que deveria executar com o elemento estando presente
// na página, ou não

setTimeout(function () {
	const elementos = document.getElementsByClassName("swal2-popup");
	// Verifica se depois de 175 ms o popup ainda é o mesmo, ou se
	// já entrou outro popup no lugar.
	// O parâmetro popup da versão normal é o parâmetro o da versão minificada.
	// A variável globalState da versão normal é a variável E da versão minificada.
	if (!elementos || !elementos.length || elementos[0] !== o)
		return;
	// Essa verificação deveria resolver algumas exceções que
	// têm aparecido esporadicamente no console depois da migração
	// para essa versão do Swal
	const copiaSwalCloseEventFinishedCallback = E.swalCloseEventFinishedCallback;
	if (copiaSwalCloseEventFinishedCallback) {
		delete E.swalCloseEventFinishedCallback;
		copiaSwalCloseEventFinishedCallback();
	}
}, 175)
